...
def flush_info():
    """从数据库查询最新消息并显示在控件上"""
    flush_dict, flush_in = select("deliverer", 'WHERE Dno=' + user_id)
    if len(flush_in) == 0:
    ...
    # data_dict_buy, flu_list = select("orderr", 'WHERE ostate = \'订单完成\' AND Dno = ' + user_id)  # 从数据库获取信息
    sql_flush = "SELECT Ono,store.sname,Omoney,Ostate,Obtime FROM orderr," \
                "store WHERE orderr.Sno=store.Sno AND orderr.dno = "+ user_id
    try:
...
def submit_order():
    '''完成订单
    首先判断配送费，是NULL则获得1元工资，否则获得相应的工资
    遍历订单列表的每一个订单，修改状态为”订单完成“
    刷新控件
        '''
    if len(order_list) == 0:
    ...
    # 修改数据库余额
    cursor.callproc('alter_del_money', args=(user_id, rest_money))
    db.commit()

    for i in order_list:
        finish_del_com = 'UPDATE orderr SET ostate = \'订单完成\' WHERE Ono = ' + cur_menu[i - 1][4]
...
def alter_info():  # 修改个人信息
    def commit_info_change():
        ...
        if altered_pwd != "":
            sql_alt += " Dpass=\'" + altered_pwd + "\'"
            changed = 1
        '''if altered_addr != "":
            if changed == 1:
                sql_alt += ','
            sql_alt += " Daddress=\'" + altered_addr + "\'"
            changed = 1'''
        if altered_tel != "":
            if changed == 1:
                sql_alt += ','
            sql_alt += " Dtel=\'" + altered_tel + "\'"
            changed = 1
        if altered_sex != "":
            if changed == 1:
                sql_alt += ','
        ...
        if altered_name != "":
            if changed == 1:
                sql_alt += ','
            sql_alt += " Dname=\'" + altered_name + "\'"
        sql_alt += "WHERE Dno=\'" + user_id + "\';"
        try:  # 执行SQL语句
    ...
    # 从数据库查询个人信息
    alter_info_dict, alter_pre_info = select("deliverer", 'WHERE Dno=' + user_id)
...
def charge_money():
    def charge_money_in():
        ...
        if user_pwd != identify_pwd:
            ...
            return
        # 修改数据库余额
        cursor.callproc('alter_del_money', args=(user_id, target_money))
        db.commit()
        flush_info()
...
def change_work_state():
    sql2 = 'WHERE Dno = ' + user_id + ';'
    if work_state == '工作':  # 当前是工作状态，说明要下班
        try:
            sql = 'UPDATE deliverer set Dstate = \'休息\''
            sql += sql2
            # 执行SQL语句
            cursor.execute(sql)
            db.commit()
        ...
        try:
            sql = 'UPDATE deliverer set Dstate = \'工作\''
            sql += sql2
            # 执行SQL语句
            cursor.execute(sql)
            db.commit()
            tk.messagebox.showinfo(title="通知", message="道路千万条\n安全第一条\n上班路上注意安全")
        except:
            db.rollback()
            tk.messagebox.showerror(message='操作失败，请稍后重试')
...
# 窗口
main_window = tk.Tk()
main_window.title('外卖管理_配送员_主界面 1.0 ')
...
tk.Label(main_window, text='最近配送完成的订单：').place(x=440, y=180)
...
class My_Tk():
    ...
    def insert_tv(self):
        ...
        # data_dict_in, cur_menu = select("view_del_unfinish")
        data_dict_in, cur_menu = select("view_del_unfinish , orderr",'WHERE view_del_unfinish.Ono = orderr.Ono AND orderr.Dno = '+user_id)
...
loggin_window.title('外卖管理_配送员_登录界面 1.0 ')
...
# 标签 用户名密码
tk.Label(loggin_window, text='骑手ID:').place(x=60, y=310)
